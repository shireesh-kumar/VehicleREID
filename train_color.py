...
# Load the data
try:
    train_loader, val_loader, test_loader = Load_Color_Train_Data()
...
best_model_path = 'rgb_color_best_resnet34_type.pth'
...
plt.ylabel('Actual')
plt.savefig('rgb_color_confusion_matrix.png')  # Save confusion matrix as an image
plt.close()
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('rgb_color_loss_plot.png')  # Save loss plot as an image
plt.close()
...
plt.ylabel('Accuracy (%)')
plt.legend()
plt.savefig('rgb_color_accuracy_plot.png')  # Save accuracy plot as an image
plt.close()
